...
export default function HwesChartTab() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const resp = await fetch(`/api/query?view=engine.tsf_vw_daily_best_hwes_a0`);
  ...
  return (
    <div>
      <h2 style={{ marginTop: 0 }}>Hwes Forecast Chart</h2>
      ...
      <div style={{width:"100%", height:"70vh"}}>
        <ResponsiveContainer>
          <LineChart data={rows}>
            ...
            <Line type="monotone" dataKey="fv" stroke="#28a745" dot={false} name="Hwes" />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
