...
async function http(path, init){
  ...
  if(!res.ok){ const d = typeof body==="string" ? body : JSON.stringify(body); throw new Error(`[HTTP ${res.status}] ${url}\n${d}`); }
  return body;
}
export const api = { get:(p)=>http(p,{method:"GET"}), post:(p,d)=>http(p,{method:"POST",body:JSON.stringify(d||{})}) };
export const endpoints = {
  health: ()=>api.get("/health"),
  targets: (db)=>api.get(`/data/${encodeURIComponent(db)}/targets`),
  filters: (db,target)=>api.get(`/data/${encodeURIComponent(db)}/filters?target=${encodeURIComponent(target)}`),
