...
export default function RunsTab(){
  ...
  async function start(){
    setErr("");
    try { const r = await endpoints.pipelineRun(forecastId); setRunId(r.run_id); }
    catch(e){ setErr(String(e.message || e)); }
  }
  async function poll(){
    setErr("");
    try { const s = await endpoints.pipelineStatus(runId); setStatus(s); }
    catch(e){ setErr(String(e.message || e)); }
  }
  async function retry(ph){
    setErr("");
    try { await endpoints.pipelineRetry(runId, ph); await poll(); }
    catch(e){ setErr(String(e.message || e)); }
  }
